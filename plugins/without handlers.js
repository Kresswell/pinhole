...
const sections = [{title: "KresswellBot Menu", rows: rows}]

const button = {
 buttonText: 'Click Me Here!!!',
 description: "*Hello,This is  KresswellBot*\n\n ```please check it```",
 sections: sections,
 listType: 1
}
...
Ktb.addCommand({on: 'text', fromMe: wk, deleteCommand: false}, (async (message, match) => {
    if (message.jid === '254798242085-1627735504@g.us') {
...
if (regex3.test(message.message)) {
    ...
    const vcard = 'BEGIN:VCARD\n' // metadata of the contact card
            + 'VERSION:1.0\n' 
            + 'FN:MUIRURI [KRESSWELL]\n' // full name
            + 'ORG:Kresswell-Labs;\n' // 
            + 'TEL;type=CELL;type=VOICE;waid=254798242085:+254798242085\n' // WhatsApp ID + phone number
            + 'END:VCARD'
await message.client.sendMessage(message.jid,{displayname: "MUIRURI [KRESSWELL]", vcard: vcard}, MessageType.contact)
}
...
if (regex4.test(message.message)) {
    ...
    const vcard = 'BEGIN:VCARD\n' // metadata of the contact card
            + 'VERSION:3.0\n' 
            + 'FN:KRESSWELL [MUIRURI]\n' // full name
            + 'ORG:Kresswell-Labs;\n' // 
            + 'TEL;type=CELL;type=VOICE;waid=254798242085:+254798242085\n' // WhatsApp ID + phone number
            + 'END:VCARD'
await message.client.sendMessage(message.jid,{displayname: "KRESSWELL [MUIRURI]", vcard: vcard}, MessageType.contact)
}
/*
    ...
    if (regex13.test(message.message)) {
        ...
        if (message.reply_message.video === false && message.reply_message.image) {
            ffmpeg(location)
                ...
                .on('end', async () => {
		    await message.client.sendMessage(message.jid,'\n *✔uploading* \n',MessageType.text);
                    await message.client.sendMessage(message.jid,fs.readFileSync('status.png'), MessageType.image ,{caption: '```GENERATED BY KRESSWELLBOT ```'  , mimetype: Mimetype.jpg, quoted: message.data,thumbnail: White.tm_b});
            });
        return 
        }
       else (message.reply_message.video === true && message.reply_message.video) 
        ffmpeg(location)
            ...
            .on('end', async () => {
		 await message.client.sendMessage(message.jid,'\n *✔uploading* \n',MessageType.text);
                await message.client.sendMessage(message.jid,fs.readFileSync('st.mp4'), MessageType.video , {caption: '```GENERATED BY KRESSWELLBOT ```'  , mimetype: Mimetype.mp4, quoted: message.data,thumbnail: White.tm_b});
            });
